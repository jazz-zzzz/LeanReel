<script lang="ts">
      ...
      queue.update(items => items.map(item => {
        if (item.id === event.payload.job_id) {
          ...
          return {
            ...item,
            progress: pct,
            status: event.payload.status as QueueStatus,
            statusText: stage === 'done'
              ? (event.payload.status === 'completed' ? '已完成' : event.payload.status)
              : `${stage}: ${pct}%`,
  ...
  function clearFinished() {
    queue.update(items => items.filter(i => i.status !== 'done' && i.status !== 'failed' && i.status !== 'cancelled'));
  }
      ...
      >
        ...
        <div class="task-progress">
          ...
          <span
            ...
            class:danger={item.status === 'failed'}
...
<style>
  ...
  .queue-row.running { border-color: var(--accent); }
