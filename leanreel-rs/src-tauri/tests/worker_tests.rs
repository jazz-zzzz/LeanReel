...
use leanreel_rs_lib::domain::traits::{Encoder, EncodingJob, EncodeOutput, MediaProber, ProgressEvent, SnapshotStore, JobId};
...
impl MediaProber for TestSyncingProber {
    ...
    fn probe_batch(&self, paths: &[PathBuf]) -> Result<Vec<ProbeResult>, String> {
        Ok(paths.iter().map(|p| {
            ...
            ProbeResult { path: p.clone(), metadata: meta }
        }).collect())
    }
}
...
impl CapturingStore {
    fn new() -> (Self, Arc<Mutex<Vec<FileSnapshot>>>) {
        ...
        (Self { captures: captures.clone() }, captures)
    }
}
impl SnapshotStore for CapturingStore {
    ...
    fn update_compression_runtime(&self, _record_id: i64, _status: &str, _progress: f64, _stage: &str, _duration_seconds: i64) -> Result<(), String> {
        Ok(())
    }
    fn finish_compression(&self, _record_id: i64, _status: &str, _progress: f64, _duration_seconds: i64, _compressed_size: i64, _error_message: &str, _sidecar_path: &str, _source_deleted: i32, _ffmpeg_command: &str) -> Result<(), String> {
        Ok(())
    }
}
