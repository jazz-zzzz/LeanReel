...
fn resolve_folder_id(store: &std::sync::MutexGuard<crate::infrastructure::db::SqliteSnapshotStore>, path: &str, folder_id: i64) -> Result<i64, String> {
    if folder_id > 0 {
...
#[tauri::command]
pub fn get_library_files(library_id: i64, state: State<AppState>) -> Result<ScanCommandResult, String> {
    let filter = crate::domain::models::FileFilter {
        library_id: if library_id > 0 { Some(library_id) } else { None },
        folder_id: None,
        probe_ok_only: None,
    };
    let snapshots = state.store.lock().map_err(|_| "store lock failed".to_string())?.query(&filter)?;
    let matcher = state.matcher.lock().map_err(|_| "matcher lock failed".to_string())?;
    Ok(build_result(&snapshots, &matcher))
}

/// Query cached file list for a folder — no scanning, instant.
#[tauri::command]
pub fn get_folder_files(folder_id: i64, state: State<AppState>) -> Result<ScanCommandResult, String> {
    let filter = crate::domain::models::FileFilter {
        library_id: None,
...
#[tauri::command]
pub fn scan_directory(path: String, folder_id: i64, state: State<AppState>) -> Result<ScanCommandResult, String> {
    let actual_folder_id = {
        let store = state.store.lock().map_err(|_| "store lock failed".to_string())?;
        resolve_folder_id(&store, &path, folder_id)?
    };

    let scanner = state.scanner.lock().map_err(|e| format!("锁获取失败: {}", e))?;
...
fn build_result(snapshots: &[crate::domain::models::FileSnapshot], matcher: &StrategyMatcher) -> ScanCommandResult {
    let files: Vec<FileEntry> = snapshots.iter().map(|s| {
...
fn format_codec(s: &crate::domain::models::FileSnapshot) -> String {
    ...
    let mut parts = vec![codec_str.to_string()];

    // Resolution label
    let h = s.video_height;
    if h >= 4320 { parts.push("8K".into()); }
    else if h >= 2160 { parts.push("4K".into()); }
    else if h >= 1440 {
        if s.video_width >= 2560 { parts.push("2K".into()); }
        else { parts.push(format!("{}p", h)); }
    }
    else if h >= 1080 { parts.push("1080p".into()); }
    else if h >= 720 { parts.push("720p".into()); }
    else if h > 0 { parts.push(format!("{}p", h)); }

    // Bitrate
    if s.bitrate_bps > 0 {
        parts.push(format!("{:.1} Mbps", s.bitrate_bps as f64 / 1_000_000.0));
    }
...
fn compute_decision(
    ...
    match m {
        ...
        crate::domain::models::StrategyResult::Encode { strategy_name, estimated_saving } => {
            let text = format!(
                "可处理: {} 节省 {:.1}-{:.1} GB",
                strategy_name,
            ...
            let full = if estimated_saving.percentage.is_empty() {
                text
            } else {
                format!("{} ({})", text, estimated_saving.percentage)
            };
