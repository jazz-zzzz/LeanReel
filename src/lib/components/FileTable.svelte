<script lang="ts">
  ...
  let sortedFiles = $derived(sortFiles($files, sortKey, sortAsc));
  let filteredFiles = $derived(sortedFiles.filter(f => {
  ...
  let selectableFiles = $derived(filteredFiles.filter(isSelectable));
  let allSelected = $derived(selectableFiles.length > 0 && selectableFiles.every(file => selectedPaths.has(file.key)));
  let visibleCount = $derived(Math.ceil((scrollEl?.clientHeight ?? 400) / ROW_H) + BUFFER * 2);
  let startIdx = $derived(Math.max(0, Math.floor(scrollTop / ROW_H) - BUFFER));
  let endIdx = $derived(Math.min(filteredFiles.length, startIdx + visibleCount));
  let visibleFiles = $derived(filteredFiles.slice(startIdx, endIdx));
  let topSpacer = $derived(startIdx * ROW_H);
  let bottomSpacer = $derived(Math.max(0, (filteredFiles.length - endIdx) * ROW_H));
  ...
  function toggleFile(file: FileEntry, event?: MouseEvent) {
    ...
    if (event?.shiftKey && selectionAnchorKey !== null) {
      selectedFilePaths.set(addSelectionRange(filteredFiles, $selectedFilePaths, selectionAnchorKey, file.key));
    } else {
  ...
  export function getSelectedPaths(): string[] { return $selectedFilePaths; }
</script>
    ...
    <div class="table-body" bind:this={scrollEl} onscroll={onScroll}>
      <table>
        ...
        <tbody>
          ...
          {#each visibleFiles as file, i (file.key)}
