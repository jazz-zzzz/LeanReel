...
class UIText:
    ...
    HISTORY_BACK = "← 返回文件列表"
    HISTORY_REFRESH = "刷新"
    HISTORY_FILTER_LIBRARY = "库:"
    HISTORY_FILTER_STRATEGY = "策略:"
    ...
    HISTORY_STATUS_DISCARDED = "已丢弃"
    HISTORY_STATUS_SKIPPED = "已跳过"
    HISTORY_TASK_RUNNING_TITLE = "任务状态"
    HISTORY_TASK_RUNNING_MSG = "任务正在{stage}，尚未完成"
    HISTORY_ENCODE_FAILED_TITLE = "转换失败"
    HISTORY_DISCARDED_TITLE = "已丢弃"
    HISTORY_FILE_NOT_FOUND_TITLE = "文件不存在"
    HISTORY_FILE_NOT_FOUND_MSG = "输出文件已不存在："
    HISTORY_FILE_NOT_FOUND_HINT = "可能原因：体积反超被丢弃 / 文件被手动删除"
    HISTORY_SUMMARY_TEMPLATE = "共 {total} 条"
    HISTORY_SUMMARY_COMPLETED = "成功 {completed}"
    HISTORY_SUMMARY_FAILED = "失败 {failed}"
    HISTORY_SUMMARY_RUNNING = "进行中 {running}"
    HISTORY_SUMMARY_SAVINGS = "累计节省 {savings}"

    # 文件列表增强
    FILE_BATCH_SELECT_HINT = "提示：按住 Shift 点击可批量勾选"
