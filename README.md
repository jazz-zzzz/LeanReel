...
| 状态 | 用途 |
| --- | --- |
| 预设策略 | 快速选择 x265 / NVENC / Copy Streams 等技术策略。 |
...
队列面板负责展示压缩任务的执行状态。失败任务会显示短错误原因，`清空已完成` 只清除完成、失败、跳过和已取消等终态任务，不会移除正在运行或等待中的任务。
...
| 策略 | 编码方向 | 预计节省 | 适合场景 |
| --- | --- | --- | --- |
| x265 HEVC CRF 18 高质量转码 | CPU HEVC | 20-35% | 画质优先，速度较慢，适合高码率 SDR H.264。 |
| x265 HEVC CRF 20 标准转码 | CPU HEVC | 35-50% | 体积和画质折中，适合多数 SDR H.264。 |
| x265 HEVC CRF 22 高压缩转码 | CPU HEVC | 50-70% | 更小体积，画质风险更高，仅建议 SDR 片源手动选择。 |
| NVENC HEVC CQ 23 高质量转码 | GPU HEVC | 25-45% | NVIDIA GPU 硬编码，速度快，画质优先。 |
| NVENC HEVC CQ 26 快速转码 | GPU HEVC | 35-55% | NVIDIA GPU 硬编码，速度很快，适合批量 SDR H.264。 |
| Copy Streams 流复制 | 视频流 copy | 5-15% | 不重编码，仅复制媒体流。 |

自动匹配会把 HEVC/H.265、HDR10、HDR10+ 和 Dolby Vision 视为优质片源，默认完全跳过，不重编码、不删流、不改容器。

自定义策略当前支持这些参数：

| 参数 | 可选值 |
| --- | --- |
| 编码器 | `libx265`、`hevc_nvenc`、`h264_nvenc`、`copy` |
| CPU 质量 | `CRF 0-35` |
| GPU 质量 | `CQ 0-51` |
| CPU 预设 | 当前自定义策略固定使用 `slow` |
| NVENC 预设 | `P1` 到 `P7` |
