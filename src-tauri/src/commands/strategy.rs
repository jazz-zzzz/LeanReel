...
fn resolve_strategy_dir() -> Result<std::path::PathBuf, String> {
    ...
    .flatten();
    for d in dirs {
        if d.exists() && d.is_dir() { return Ok(d); }
    }
...
pub fn load_strategies(state: State<AppState>) -> Result<StrategyListResult, String> {
    ...
    let items: Vec<StrategyItem> = strategies.iter().map(|s| StrategyItem {
        name: s.name.clone(),
    ...
    let mut matcher = state.matcher.lock().map_err(|_| "策略锁获取失败".to_string())?;
    *matcher = crate::services::matcher::StrategyMatcher::new(strategies);

    Ok(StrategyListResult { count, message: format!("已加载 {} 个策略", count), strategies: items })
}

#[tauri::command]
pub fn save_strategy(name: String, strategy_json: String, state: State<AppState>) -> Result<(), String> {
...
pub fn delete_strategy(name: String, state: State<AppState>) -> Result<(), String> {
    let dir = resolve_strategy_dir()?;
    let path = dir.join(format!("{}.json", name));
    if path.exists() { fs::remove_file(&path).map_err(|e| format!("删除策略失败: {}", e))?; }
    let strategies = load_strategies_from_disk()?;
    if let Ok(mut matcher) = state.matcher.lock() {
        *matcher = crate::services::matcher::StrategyMatcher::new(strategies);
    }
...
#[tauri::command]
pub fn save_strategy_order(order: Vec<SortOrderEntry>, state: State<AppState>) -> Result<(), String> {
    let dir = resolve_strategy_dir()?;
    for entry in &order {
        ...
        if let Ok(json) = fs::read_to_string(&path) {
            if let Ok(mut strategy) = serde_json::from_str::<Strategy>(&json) {
                ...
                let new_json = serde_json::to_string_pretty(&strategy).map_err(|e| e.to_string())?;
