...
impl Encoder for SpyEncoder {
    fn run(
        ...
        *self.last_job.lock().unwrap() = Some(job.clone());
        ...
        std::fs::write(
            &job.output_path,
            b"compressed content here for leanreel lifecycle test",
        )
...
fn test_complete_encode_lifecycle_via_submit_encode() {
    ...
    let history = verify_store
        ...
        .expect("查询压缩历史失败");
    assert!(!history.is_empty(), "compression_history 应有记录");
...
fn test_encode_with_dolby_vision_flag() {
    ...
    assert!(
        last_job.has_dolby_vision,
        "DV 任务应设置 has_dolby_vision=true"
    );
