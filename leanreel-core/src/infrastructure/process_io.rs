...
pub fn is_smb_path(path: &Path) -> bool {
    ...
    // Linux CIFS/SMB mount detection via /proc/mounts
    #[cfg(target_os = "linux")]
    {
        return is_cifs_mountpoint(path);
    }
...
#[cfg(target_os = "linux")]
fn is_cifs_mountpoint(path: &Path) -> bool {
    ...
    for line in mounts.lines() {
        ...
        if parts.len() >= 3 {
            let fs_type = parts[2];
            if fs_type == "cifs" || fs_type == "smb3" || fs_type == "smbfs" {
                let mount_point = std::path::Path::new(parts[1]);
