...
pub fn get_queue_status(state: &AppState) -> Result<serde_json::Value, String> {
    Ok(
        serde_json::json!({ "paused": state.worker.is_paused(), "cancelled": state.worker.is_cancelled() }),
    )
}

pub fn pause_encode(state: &AppState) -> Result<(), String> {
    state.worker.pause();
    Ok(())
}

pub fn resume_encode(state: &AppState) -> Result<(), String> {
    state.worker.resume();
    Ok(())
}

pub fn cancel_encode(state: &AppState) -> Result<(), String> {
    state.ffmpeg.cancel().ok();
    state.worker.cancel();
    Ok(())
}

pub fn cancel_task(job_id: &str, state: &AppState) -> Result<(), String> {
