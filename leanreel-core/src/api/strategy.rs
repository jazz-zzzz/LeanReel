...
pub fn load_strategies_from_disk() -> Result<Vec<crate::domain::models::Strategy>, String> {
    ...
    for entry in std::fs::read_dir(&dir).map_err(|e| format!("读取失败: {}", e))? {
        let entry = entry.map_err(|_| "读取文件失败".to_string())?;
        let path = entry.path();
        if path.extension().and_then(|e| e.to_str()) == Some("json") {
            let json = std::fs::read_to_string(&path).map_err(|e| e.to_string())?;
            let s: crate::domain::models::Strategy =
...
pub fn load_strategies(state: &AppState) -> Result<StrategyListResult, String> {
...
pub fn save_strategy(name: &str, strategy_json: &str, state: &AppState) -> Result<(), String> {
    let dir = resolve_strategy_dir()?;
    let path = dir.join(format!("{}.json", name));
    std::fs::write(&path, strategy_json).map_err(|e| format!("写入策略失败: {}", e))?;
    let strategies = load_strategies_from_disk()?;
...
pub fn delete_strategy(name: &str, state: &AppState) -> Result<(), String> {
    let dir = resolve_strategy_dir()?;
    let path = dir.join(format!("{}.json", name));
    if path.exists() {
        std::fs::remove_file(&path).map_err(|e| format!("删除策略失败: {}", e))?;
    }
    let strategies = load_strategies_from_disk()?;
    if let Ok(mut matcher) = state.matcher.lock() {
        *matcher = crate::services::matcher::StrategyMatcher::new(strategies);
...
pub fn save_strategy_order(order: &[SortOrderEntry], state: &AppState) -> Result<(), String> {
    let dir = resolve_strategy_dir()?;
    for entry in order {
        let path = dir.join(format!("{}.json", entry.name));
        if let Ok(json) = std::fs::read_to_string(&path) {
            if let Ok(mut strategy) =
                serde_json::from_str::<crate::domain::models::Strategy>(&json)
            {
